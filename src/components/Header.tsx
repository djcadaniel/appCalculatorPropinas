...
type HeaderProps = {
  ...
  order: OrderItem[]
  showCar: boolean
  showDetailsScreen: React.MouseEventHandler<HTMLDivElement> | undefined
}

export default function Header({showDetails, addItem, showCar, showDetailsScreen} : HeaderProps) {
  ...
  return (
    <header className={`contain h-full w-full`}>
      ...
      />
      <div onClick={showDetailsScreen} className={`${ showCar ? 'blur-sm backdrop-blur-sm' : ''}`}>
        ...
        <div className={`px-10 w-full h-[calc(100vh-225px)] flex flex-col sm:flex-row justify-center items-center font-montserrat ${ showCar ? 'blur-sm backdrop-blur-sm' : ''} `}>
